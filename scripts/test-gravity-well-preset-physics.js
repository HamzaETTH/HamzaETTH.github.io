...
async function main() {
  ...
  const result = {
    ...
    gates: 'Desktop: no nominal centered edge contacts and at least 99% ordinary scattered retention. Touch/mobile: no edge contacts for nominal particles seeded inside black-well capture regions; full centered/scattered cohorts are diagnostic.',
    ...
    for (const viewport of matrix) {
      ...
      for (const id of ids) {
        for (const cohort of viewport.mobile ? cohorts.concat('captured') : cohorts) {
          ...
          if (!state.mobile && cohort === 'scattered' && state.retention < 0.99) failures.push('ordinary scattered retention below 99%');
