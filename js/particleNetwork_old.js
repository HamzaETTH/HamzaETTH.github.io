function PerformanceMonitor(container) {
  ...
  // Optional: Store frame times to calculate average frame render time
  this.frameTimes = [];
}
...
(function (a) {
  ...
  c.prototype.update = function (attractiveForce, repulsiveForce, forceDistance, forceStrength) {
    ...
    if (this.particleNetwork && this.particleNetwork.o) {
      this.particleNetwork.o.forEach((otherParticle) => {
        if (otherParticle !== this) {
          var dx = otherParticle.x - this.x;
          var dy = otherParticle.y - this.y;
          var distance = Math.sqrt(dx * dx + dy * dy);

          if (distance < this.options.particleInteractionDistance) {
            var forceDirectionX = dx / distance;
            var forceDirectionY = dy / distance;
            var force =
              (this.options.particleRepulsionForce * (this.options.particleInteractionDistance - distance)) /
  ...
  var applyParticleInteraction = function (
    ...
    particles.forEach(function (particle) {
      ...
      if (distance < particleInteractionDistance && distance > 0) {
        var forceDirectionX = dx / distance;
        var forceDirectionY = dy / distance;
  ...
  return (
    (b = function (a, b) {
      (this.i = a),
        ...
        (this.options = {
          ...
          lineCyclingSpeed: void 0 !== b.lineCyclingSpeed ? b.lineCyclingSpeed : 40,
    ...
    (b.prototype.update = function () {
      ...
      if (this.options.lineColorCycling) {
        ...
        const cyclingSpeed = this.options.lineCyclingSpeed;

        // Initialize hues if they don't exist
        if (this.lineHue1 === undefined) {
          this.lineHue1 = generateRandomColor();
          this.lineHue2 = generateDistinctColor(this.lineHue1, minDifference);
        }
        ...
        let diff = Math.abs(this.lineHue1 - this.lineHue2);
        diff = diff > 180 ? 360 - diff : diff;

        if (diff < minDifference) {
          // Adjust hue2 gradually
          const adjustment = (minDifference - diff) * 0.1; // Gradual adjustment
      ...
      for (var a = 0; a < this.o.length; a++) {
        ...
        for (var b = this.o.length - 1; b > a; b--) {
          ...
          if (c < this.options.particleInteractionDistance) {
          ...
          if (c <= this.options.lineConnectionDistance) {
            ...
            this.g.beginPath(),
              ...
              (this.g.globalAlpha = (this.options.lineConnectionDistance - c) / this.options.lineConnectionDistance),
...
var options = {
  ...
  // Explosion options
  particleInteractionDistance: 2,
  particleRepulsionForce: 1,
